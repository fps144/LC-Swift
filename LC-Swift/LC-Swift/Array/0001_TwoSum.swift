...
//        // 思路：遍历数据源，检查数据源中是否存在两个值，一个是游标值 num，另一个是 target - num。因为不能重复利用相同元素，所以从首尾分别取第一个值，使其满足上面的关系，将符合条件的 index 放入数组返回。
//        // 超时原因：遍历数据源时每循环一次都要再次遍历两遍数据源，耗时严重。
//        // 改进方案：减少数据源遍历次数，根据题目中 value 和 index 的关系，可以尝试建立 value 和 index 的映射。
//        var result = [Int]()
//        for num in nums {
